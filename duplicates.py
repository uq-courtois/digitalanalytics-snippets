...
print(duplicated((subset=['title','year','duration']).sum()) # Number of duplicates with same title, year, duration
df = df.drop_duplicates(subset=['title','year','duration']) # Drop these duplicates

[df['title'] == 'Lady and the Tramp'][['title','year']] # lady and the Tramp after
